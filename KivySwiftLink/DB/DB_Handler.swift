...
func load_global(realm: Realm) -> GlobalSettings {
    ...
    //try! realm.write({
        ...
        global.python_version.append(objectsIn: [3,9,2])
...
class ProjectHandler {
    ...
    init(db_path: String!) {
        var url: URL
        if let path = db_path {
            url = URL(fileURLWithPath: path).appendingPathComponent("db.realm")
        } else {
            url = URL(fileURLWithPath: root_path).appendingPathComponent("db.realm")
        }
